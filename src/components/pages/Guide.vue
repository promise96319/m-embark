...
<section class="project">
  ...
  <main class="project-info">
    ...
    <section class="project-main">
      <van-row gutter="20">
        <van-col v-for="(item, index) in bigCate" :key="index" :class="{fontStyle: index === 2}">
...
export default {
  ...
  mounted () {
    // 请求或者从store里得到数据
    if (!this.$store.getters.getProjects('guide').length){
      axios.get(URL.allProjects)
      .then((res) => {
        this.saveTotalProjects(res.data.projects)
      }).then(() => {
        this.projectData = this.$store.getters.getProjects('guide')
      })
      .catch((err) => {
