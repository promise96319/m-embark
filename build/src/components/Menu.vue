...
<aside class="menu">
  ...
  <section class="category">
    ...
    <router-link :to="{ name: 'Project', params: {id: '科研项目'} }">
      <div @click="selectEvent">科研项目</div>
    </router-link>

    <router-link :to="{ name: 'Project', params: {id: '竞赛指导'} }">
      <div @click="selectEvent">竞赛指导</div>
    </router-link>

    <router-link :to="{ name: 'Project', params: {id: 'AI入门'} }">
...
export default {
  ...
  methods: {
    selectEvent(e) {
      this.handleMenuShow()
      if (['科研项目', '竞赛指导', 'AI入门'].includes(e.target.innerHTML)) {
        this.changeProjectCate(e.target.innerHTML)
      }
    },
    ...mapMutations(['changeProjectCate', 'handleMenuShow']),
    handleMenuFullPage() {
